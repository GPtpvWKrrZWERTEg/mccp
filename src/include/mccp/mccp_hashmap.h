#ifndef __MCCP_HASHMAP_H__
#define __MCCP_HASHMAP_H__
...
#define MCCP_HASHMAP_TYPE_STRING	0U
#define MCCP_HASHMAP_TYPE_ONE_WORD \
...
typedef unsigned int mccp_hashmap_type_t;


typedef struct HashEntry *	mccp_hashentry_t;
typedef struct mccp_hashmap_record *	mccp_hashmap_t;
 ...
 */
typedef bool
(*mccp_hashmap_iteration_proc_t)(void *key, void *val,
                                 mccp_hashentry_t he,
                                 void *arg);
 ...
 */
typedef void
(*mccp_hashmap_value_freeup_proc_t)(void *value);
 ...
 *	functions invoked via the mccp_hashmap_iterate().
 */
void
mccp_hashmap_set_value(mccp_hashentry_t he,
                       void *val);
 ...
 *	(\b MCCP_HASHMAP_TYPE_STRING: string key,
 *	\b MCCP_HASHMAP_TYPE_ONE_WORD: 64/32 bits unsigned integer key)
 ...
 *
 *	@details The \b proc is called when mccp_hashmap_destroy()
 ...
 *	\b (mccp_hashmap_type_t)(sizeof(int64_t) / sizeof(int))
 */
mccp_result_t
 ...
 *	mccp_hashmap_shutdown() if needed.
 */
void
mccp_hashmap_destroy(mccp_hashmap_t *hmptr,
                     bool free_values);
 ...
 */
mccp_result_t
mccp_hashmap_clear(mccp_hashmap_t *hmptr,
                   bool free_values);
 ...
 *
 *	@retval	MCCP_RESULT_OK		Succeeded, found the value.
 *	@retval MCCP_RESULT_NOT_FOUND	Failed, the value not found.
 *	@retval MCCP_RESULT_NOT_OPERATIONAL	Failed, not operational.
 *	@retval MCCP_RESULT_ANY_FAILURES	Failed.
 */
mccp_result_t
mccp_hashmap_find(mccp_hashmap_t *hmptr,
                  void *key,
                  void **valptr);
 ...
 *
 *	@retval MCCP_RESULT_OK		Succeeded, the value newly
 *	added.
 *	@retval MCCP_RESULT_INVALID_ARGS	Failed, invalid argument(s).
 *	@retval MCCP_RESULT_ALREADY_EXISTS	Failed, the key - value pair
 *	already exists.
 *	@retval MCCP_RESULT_NO_MEMORY	Failed, no memory.
 *	@retval MCCP_RESULT_NOT_OPERATIONAL	Failed, not operational.
 *	@retval MCCP_RESULT_ANY_FAILURES	Failed.
 ...
 *	returns \b MCCP_RESULT_OK, the \b *valptr is set to \b NULL
 ...
 *	mccp_hashmap_find() to check the existence of the entry
 *	before the addition.
 *
 ...
 *	returns \b MCCP_RESULT_OK (note that the \b allow_overwrite
 *	is \b true for this case), it could be needed to free the
 *	value up.
 *
 *	@details If the \b allow_overwrite is \b false and the
 *	functions returns \b MCCP_RESULT_ALREADY_EXISTS, nothing is
 ...
 *	calling of the mccp_hashmap_create() is not \b NULL.
 *
 *	@retval MCCP_RESULT_OK Succeeded, the pair deleted, or the
 *	pair doesn't exist.
 *	@retval MCCP_RESULT_INVALID_ARGS	Failed, invalid argument(s).
 *	@retval MCCP_RESULT_NOT_OPERATIONAL	Failed, not operational.
 *	@retval MCCP_RESULT_ANY_FAILURES	Failed.
 ...
 *	calling the mccp_hashmap_set_value() is allowed in the
 *	functions.
 *
 *	@retval	MCCP_RESULT_OK		Succeeded.
 *	@retval MCCP_RESULT_ITERATION_HALTED The iteration was
 ...
 */
mccp_result_t
mccp_hashmap_statistics(mccp_hashmap_t *hmptr,
                        const char **msgptr);
