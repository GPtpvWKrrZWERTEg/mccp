#ifndef __MCCP_MACROS_H__
#define __MCCP_MACROS_H__





#include "mccp_config.h"
...
#define PRINT_FORTMAT_SIZE_T	z

#else

#if SIZEOF_SIZE_T == SIZEOF_LONG_LONG || SIZEOF_SIZE_T == SIZEOF_INT64_T
#define PRINT_FORTMAT_SIZE_T	PRINT_FORMAT_64
#elif SIZEOF_SIZE_T == SIZEOF_LONG_INT
#define PRINT_FORTMAT_SIZE_T	PRINT_FORMAT_LONG
#elif SIZEOF_SIZE_T == SIZEOF_INT
#define PRINT_FORTMAT_SIZE_T	d
#endif /* SIZEOF_SIZE_T == SIZEOF_LONG_LONG ... */

#endif /* HAVE_PRINT_FORMAT_FOR_SIZE_T */

#define PFSZ(c)	STRINGIFY(%) MACRO_STRINGIFY(PRINT_FORTMAT_SIZE_T) \
    MACRO_STRINGIFY(c)
#define PFSZS(s, c) \
    STRINGIFY(%) MACRO_STRINGIFY(s) MACRO_STRINGIFY(PRINT_FORTMAT_SIZE_T) \
    MACRO_STRINGIFY(c)
...
#define IS_MCCP_RESULT_OK(s)	(((s) >= 0) ? true : false)
...
#define TS_TO_NSEC(ts) \
  ((mccp_chrono_t)((ts).tv_sec) * 1000LL * 1000LL * 1000LL + \
   (mccp_chrono_t)(ts).tv_nsec)

#define TV_TO_NSEC(tv) \
  (((mccp_chrono_t)((tv).tv_sec) * 1000LL * 1000LL + \
    (mccp_chrono_t)(tv).tv_usec) * 1000LL)
