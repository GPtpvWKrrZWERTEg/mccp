...
static mccp_hashmap_t s_mult_tbl;
...
s_once_proc(void) {
  ...
  if (mpz_init_set_str(&s_0, "0", 10) != 0) {
    mccp_exit_fatal("can't initialize '0' to an MP_INT.\n");
  }
  if (mpz_init_set_str(&s_1, "1", 10) != 0) {
    mccp_exit_fatal("can't initialize '1' to an MP_INT.\n");
  }
  ...
  if ((r = mccp_hashmap_create(&s_mult_tbl,
                               MCCP_HASHMAP_TYPE_STRING, NULL)) !=
      MCCP_RESULT_OK) {
    mccp_perror(r, "mccp_hashmap_create()");
    mccp_exit_fatal("can't initialize an SI prefix multiplier table.\n");
  }

  for (i = 0; i < 8; i++) {
    if (mpz_init_set_str(&(s_mult[i]), "1000", 10) != 0) {
      mccp_exit_fatal("can't initialize '1000' to an MP_INT.\n");
    }
    if (mpz_init_set_str(&(s_mult_i[i]), "1024", 10) != 0) {
      mccp_exit_fatal("can't initialize '1024' to an MP_INT.\n");
    }
    for (j = 0; j < i; j++) {
  ...
  v = (void *)&(s_mult[0]);
  (void)mccp_hashmap_add(&s_mult_tbl, (void *)"k", &v, false);
  v = (void *)&(s_mult_i[0]);
  (void)mccp_hashmap_add(&s_mult_tbl, (void *)"ki", &v, false);

  v = (void *)&(s_mult[1]);
  (void)mccp_hashmap_add(&s_mult_tbl, (void *)"m", &v, false);
  v = &(s_mult_i[1]);
  (void)mccp_hashmap_add(&s_mult_tbl, (void *)"mi", &v, false);

  v = (void *)&(s_mult[2]);
  (void)mccp_hashmap_add(&s_mult_tbl, (void *)"g", &v, false);
  v = (void *)&(s_mult_i[2]);
  (void)mccp_hashmap_add(&s_mult_tbl, (void *)"gi", &v, false);

  v = (void *)&(s_mult[3]);
  (void)mccp_hashmap_add(&s_mult_tbl, (void *)"t", &v, false);
  v = (void *)&(s_mult_i[3]);
  (void)mccp_hashmap_add(&s_mult_tbl, (void *)"ti", &v, false);

  v = (void *)&(s_mult[4]);
  (void)mccp_hashmap_add(&s_mult_tbl, (void *)"p", &v, false);
  v = (void *)&(s_mult_i[4]);
  (void)mccp_hashmap_add(&s_mult_tbl, (void *)"pi", &v, false);

  v = (void *)&(s_mult[5]);
  (void)mccp_hashmap_add(&s_mult_tbl, (void *)"x", &v, false);
  v = (void *)&(s_mult_i[5]);
  (void)mccp_hashmap_add(&s_mult_tbl, (void *)"xi", &v, false);

  v = (void *)&(s_mult[6]);
  (void)mccp_hashmap_add(&s_mult_tbl, (void *)"z", &v, false);
  v = (void *)&(s_mult_i[6]);
  (void)mccp_hashmap_add(&s_mult_tbl, (void *)"zi", &v, false);

  v = (void *)&(s_mult[7]);
  (void)mccp_hashmap_add(&s_mult_tbl, (void *)"y", &v, false);
  v = (void *)&(s_mult_i[7]);
  (void)mccp_hashmap_add(&s_mult_tbl, (void *)"yi", &v, false);


  snprintf(buf, sizeof(buf), "%d", SHRT_MIN);
  if (mpz_init_set_str(&s_SHRT_MIN, buf, 10) != 0) {
    mccp_exit_fatal("can't initialize SHRT_MIN to an MP_INT.\n");
  }
  snprintf(buf, sizeof(buf), "%d", SHRT_MAX);
  if (mpz_init_set_str(&s_SHRT_MAX, buf, 10) != 0) {
    mccp_exit_fatal("can't initialize SHRT_MAX to an MP_INT.\n");
  }
  snprintf(buf, sizeof(buf), "%u", USHRT_MAX);
  if (mpz_init_set_str(&s_USHRT_MAX, buf, 10) != 0) {
    mccp_exit_fatal("can't initialize USHRT_MAX to an MP_INT.\n");
  }

  snprintf(buf, sizeof(buf), "%d", INT_MIN);
  if (mpz_init_set_str(&s_INT_MIN, buf, 10) != 0) {
    mccp_exit_fatal("can't initialize INT_MIN to an MP_INT.\n");
  }
  snprintf(buf, sizeof(buf), "%d", INT_MAX);
  if (mpz_init_set_str(&s_INT_MAX, buf, 10) != 0) {
    mccp_exit_fatal("can't initialize INT_MAX to an MP_INT.\n");
  }
  snprintf(buf, sizeof(buf), "%u", UINT_MAX);
  if (mpz_init_set_str(&s_UINT_MAX, buf, 10) != 0) {
    mccp_exit_fatal("can't initialize UINT_MAX to an MP_INT.\n");
  }

  snprintf(buf, sizeof(buf), "%ld", LONG_MIN);
  if (mpz_init_set_str(&s_LONG_MIN, buf, 10) != 0) {
    mccp_exit_fatal("can't initialize LONG_MIN to an MP_INT.\n");
  }
  snprintf(buf, sizeof(buf), "%ld", LONG_MAX);
  if (mpz_init_set_str(&s_LONG_MAX, buf, 10) != 0) {
    mccp_exit_fatal("can't initialize LONG_MAX to an MP_INT.\n");
  }
  snprintf(buf, sizeof(buf), "%lu", ULONG_MAX);
  if (mpz_init_set_str(&s_ULONG_MAX, buf, 10) != 0) {
    mccp_exit_fatal("can't initialize ULONG_MAX to an MP_INT.\n");
  }

  snprintf(buf, sizeof(buf), "%lld", LLONG_MIN);
  if (mpz_init_set_str(&s_LLONG_MIN, buf, 10) != 0) {
    mccp_exit_fatal("can't initialize LLONG_MIN to an MP_INT.\n");
  }
  snprintf(buf, sizeof(buf), "%lld", LLONG_MAX);
  if (mpz_init_set_str(&s_LLONG_MAX, buf, 10) != 0) {
    mccp_exit_fatal("can't initialize LLONG_MAX to an MP_INT.\n");
  }
  snprintf(buf, sizeof(buf), "%llu", ULLONG_MAX);
  if (mpz_init_set_str(&s_ULLONG_MAX, buf, 10) != 0) {
    mccp_exit_fatal("can't initialize ULLONG_MAX to an MP_INT.\n");
  }
...
s_ctors(void) {
  s_init();
  mccp_msg_debug(5, "The string utility module initialized.\n");
}
...
s_final(void) {
  ...
  mccp_hashmap_destroy(&s_mult_tbl, true);
}
...
s_dtors(void) {
  s_final();
  mccp_msg_debug(5, "The string utility module finalized.\n");
}
...
s_get_prefix_multiplier(char *str) {
  ...
  if (len > 0) {
    ...
    if (key != NULL) {
      void *val = NULL;
      mccp_result_t r;
      if ((r = mccp_hashmap_find(&s_mult_tbl, (void *)key, &val)) ==
          MCCP_RESULT_OK &&
          val != NULL) {
        ret = val;
      } else {
        mccp_exit_fatal("can't find a multiplier for '%s', "
                        "must not happen.\n", key);
      }
    } else {
      ret = &s_1;
...
mccp_str_parse_long_double(const char *buf, long double *val) {
  ...
  ret = mccp_str_parse_double(buf, &d);
          ...
          if (*buf != '\0') {
            ...
            while (*buf != '\0') {
              ...
              if (buf != NULL) {
                ...
                n = MCCP_RESULT_QUOTE_NOT_CLOSED;
                goto done;
              }
            }
          } else {
