...
s_test_thread_finalize(const mccp_thread_t *tptr, bool is_canceled,
  ...
  if (tptr != NULL) {
    ...
    if (tt != NULL) {
      mccp_mutex_lock(&(tt->m_lock));
      {
        tt->m_is_operational = false;
      }
      mccp_mutex_unlock(&(tt->m_lock));

      mccp_msg_debug(1, "enter (%s).\n",
...
s_test_thread_destroy(const mccp_thread_t *tptr, void *arg) {
  ...
  if (tptr != NULL) {
    ...
    if (tt != NULL) {
      (void)mccp_mutex_lock(&(tt->m_lock));
      {
        mccp_msg_debug(1, "enter.\n");
        if (tt->m_is_operational == false) {
          mccp_msg_debug(1, "freeup.\n");
          free((void *)tt->m_data);
          tt->m_data = NULL;
        }
        s_is_deleted = true;
      }
      (void)mccp_mutex_unlock(&(tt->m_lock));
    }

    mccp_mutex_destroy(&(tt->m_lock));
  }
}
  ...
  if (tptr != NULL) {
    ...
    if (tt != NULL) {
      while (tt->m_stop_requested == false) {
        mccp_msg_debug(1, "will sleep a sec...\n");
        (void)mccp_chrono_nanosleep(1000 * 1000 * 1000, NULL);
      }
      ret = MCCP_RESULT_OK;
    }
  }
...
s_initialize(test_thread_t tt, size_t n) {
  ...
  if (tt != NULL) {
    ...
    if (tt->m_data != NULL) {
      mccp_result_t r;
      if ((r = mccp_mutex_create(&(tt->m_lock))) != MCCP_RESULT_OK) {
...
test_thread_create(test_thread_t *ttptr, size_t n) {
  ...
  if (ttptr != NULL) {
    ...
    if (*ttptr == NULL && ret == true) {
      *ttptr = tt;
      (void)mccp_thread_free_when_destroy((mccp_thread_t *)&tt);
    }
  }
...
main(int argc, const char *const argv[]) {
  ...
  if (test_thread_create(&tt, 100) != true) {
    mccp_exit_fatal("Can't create a test thread object.\n");
  }
  if (mccp_thread_start((mccp_thread_t *)&tt, false) !=
      MCCP_RESULT_OK) {
    mccp_exit_fatal("Can't start a thread.\n");
  }
  mccp_chrono_nanosleep(5LL * 1000LL * 1000LL * 1000LL, NULL);
  test_thread_request_stop(tt);
  if (mccp_thread_wait((mccp_thread_t *)&tt, -1) !=
      MCCP_RESULT_OK) {
    mccp_exit_fatal("Can't wait the thread.\n");
  }
  ...
  tt = &ttr;
  if (test_thread_create(&tt, 100) != true) {
    mccp_exit_fatal("Can't initialize a test thread object.\n");
  }
  if (mccp_thread_start((mccp_thread_t *)&tt, false) !=
      MCCP_RESULT_OK) {
    mccp_exit_fatal("Can't start a thread.\n");
  ...
  mccp_chrono_nanosleep(5LL * 1000LL * 1000LL * 1000LL, NULL);
  if (mccp_thread_cancel((mccp_thread_t *)&tt) != MCCP_RESULT_OK) {
    mccp_exit_fatal("Can't cancel the thread.\n");
  }
  if (mccp_thread_wait((mccp_thread_t *)&tt, -1) !=
      MCCP_RESULT_OK) {
    mccp_exit_fatal("Can't wait the thread.\n");
  }
  if (mccp_thread_is_canceled((mccp_thread_t *)&tt, &is_canceled) !=
      MCCP_RESULT_OK) {
    mccp_exit_fatal("The returned value must be \"canceled\".\n");
  }
  mccp_thread_destroy((mccp_thread_t *)&tt);
  ...
  if (mccp_thread_start((mccp_thread_t *)&tt, true) !=
      MCCP_RESULT_OK) {
    mccp_exit_fatal("Can't start a thread.\n");
  }
  mccp_chrono_nanosleep(5LL * 1000LL * 1000LL * 1000LL, NULL);
  test_thread_request_stop(tt);
  mccp_chrono_nanosleep(2LL * 1000LL * 1000LL * 1000LL, NULL);
  if (s_is_deleted == false) {
    mccp_exit_fatal("The thread object must be freed and "
  ...
  if (mccp_thread_wait((mccp_thread_t *)&tt,
                       5LL * 1000LL * 1000LL * 1000LL) !=
      MCCP_RESULT_TIMEDOUT) {
    mccp_exit_fatal("Must be timed out.\n");
  }
  test_thread_request_stop(tt);
  if (mccp_thread_wait((mccp_thread_t *)&tt, -1) !=
      MCCP_RESULT_OK) {
