...
static mccp_mutex_t start_lock = NULL;
static mccp_mutex_t stop_lock = NULL;


static test_thread_record *tts = NULL;
static size_t n_created_thds = 0;
static mccp_bbq_t *bbqs = NULL;
static size_t n_created_bbqs = 0;
static mccp_qmuxer_poll_t *polls = NULL;
static size_t n_created_polls = 0;
static mccp_qmuxer_t qmx = NULL;
...
s_stop_all(void) {
  if (is_child == true) {
    ...
    if (stop_lock != NULL) {
      ...
      (void)mccp_mutex_lock(&stop_lock);
      {
        if (is_stopped == false) {
          ...
          if (bbqs != NULL) {
            for (i = 0; i < n_created_bbqs; i++) {
              mccp_bbq_shutdown(&(bbqs[i]), true);
            }
          }
        }
      }
      (void)mccp_mutex_unlock(&stop_lock);
    ...
    (void)mccp_mutex_lock(&stop_lock);
    {
      ...
      if (tts != NULL) {
        for (i = 0; i < n_created_thds; i++) {
          tt = &(tts[i]);
          (void)mccp_thread_wait((mccp_thread_t *)&tt, -1LL);
          (void)mccp_thread_destroy((mccp_thread_t *)&tt);
        }
        free((void *)tts);
        tts = NULL;
      }
...
do_run(size_t nthds, ssize_t nputs) {
  ...
  if ((r = mccp_mutex_create(&stop_lock)) != MCCP_RESULT_OK) {
    mccp_perror(r, "mccp_mutex_create()");
    goto done;
  }
          ...
          for (j = 0; j < (size_t)qsz; j++) {
            ...
            nullify:
              if ((r = mccp_qmuxer_poll_set_queue(&(polls[i]), NULL)) ==
                  MCCP_RESULT_OK) {
                mccp_msg_debug(1, "Q" PFSZS(04, u) " is not valid "
...
s_sighandler(int sig) {
  mccp_msg_debug(5, "got a signal %d\n", sig);
  if (sig == SIGINT) {
    mccp_msg_debug(5, "WILL EXIT.\n");

    is_signaled = true;
    s_stop_all();

    mccp_msg_debug(5, "STOPPED ALL.\n");
  }
}
...
main(int argc, const char *const argv[]) {
  ...
  (void)mccp_signal(SIGHUP, s_sighandler, NULL);
  (void)mccp_signal(SIGINT, s_sighandler, NULL);
